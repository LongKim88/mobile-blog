...
**"이 작업을 매주 월요일·목요일 오전 8시(KST)에 실행되도록 스케줄 태스크로 등록해 줘"**
라고 요청하면 됩니다.
...
기본은 IT 위주, Book은 2주에 한 번 목요일 자리에 들어갑니다.

| 요일 | 카테고리 | 형식 | 분량 | 표지 |
|------|----------|------|------|------|
| 월요일 (매주) | IT | 도구·오픈소스 소개 또는 주간 IT 뉴스 큐레이션 | 1200~1800자 | placeholder 또는 도구 로고 |
| 목요일 (홀수주) | IT | IT 뉴스 큐레이션 5건 (각 2~3문장 + 한 줄 의견) | 800~1200자 | placeholder |
| 목요일 (짝수주) | Book | 책 1권 핵심 요약·서평 (3개 메시지 + 누구에게 추천) | 1500~2500자 | 도서 표지 또는 placeholder |

#### Book 차례 판별

목요일 routine 시작 시 다음 SQL로 가장 최근 Book 글의 발행일 조회:

```bash
curl "$SUPABASE_URL/rest/v1/posts?category=eq.Book&status=eq.published&order=published_at.desc&limit=1&select=published_at" \
  -H "apikey: $SUPABASE_SERVICE_ROLE_KEY" \
  -H "Authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY"
```

- 결과가 비어 있거나 마지막 Book 발행일이 **14일 이상 지났으면** → 이번 목요일은 **Book** 차례
- 14일 미만이면 → **IT 뉴스** 차례

이 방식이 ISO 주차 짝/홀수보다 견고합니다 (격주 어긋날 때 자연 회복).
